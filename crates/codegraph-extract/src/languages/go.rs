...
pub struct GoExtractor { lang: tree_sitter::Language }

impl GoExtractor {
    pub fn new() -> Self { Self { lang: tree_sitter_go::LANGUAGE.into() } }
}
...
fn walk(node: &Node, ctx: &mut Ctx) {
    ...
    match node.kind() {
        "function_declaration" => { pushed = push_named(ctx, node, NodeKind::Function); }
        "method_declaration" => { pushed = push_named(ctx, node, NodeKind::Method); }
        "type_spec" => {
        ...
        "const_spec" => { pushed = push_named(ctx, node, NodeKind::Constant); }
        "var_spec" => { pushed = push_named(ctx, node, NodeKind::Variable); }
        "import_spec" => { emit_import(node, ctx); }
...
fn push_named(ctx: &mut Ctx, node: &Node, kind: NodeKind) -> Option<usize> {
    let name_node = node.child_by_field_name("name").or_else(|| {
        ...
        for ch in node.children(&mut c) {
            if matches!(ch.kind(), "identifier" | "field_identifier" | "type_identifier") { found = Some(ch); break; }
        }
        found
    })?;
    ...
    ctx.result.nodes.push(NodeDraft {
        ...
        signature: sig, docstring: None, language: "go".into(),
    });
...
fn emit_call(node: &Node, ctx: &mut Ctx) {
    ...
    let name = match callee.kind() {
        ...
        "selector_expression" => callee.child_by_field_name("field").and_then(|f| f.utf8_text(ctx.src).ok()).map(|s| s.to_string()),
        _ => None,
    };
...
fn emit_import(node: &Node, ctx: &mut Ctx) {
    ...
    let Ok(text) = path.utf8_text(ctx.src) else { return };
    let module = text.trim_matches('"').to_string();
    let from = ctx.parent_idx.unwrap_or(usize::MAX);
    if from == usize::MAX { return; }
    ctx.result.imports.push(RawImport { from_idx: from, module, line: node.start_position().row as u32 + 1 });
}
