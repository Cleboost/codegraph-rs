...
pub struct JavaScriptExtractor { lang: tree_sitter::Language }

impl JavaScriptExtractor {
    pub fn new() -> Self { Self { lang: tree_sitter_javascript::LANGUAGE.into() } }
}
...
fn walk(node: &Node, ctx: &mut Ctx) {
    ...
    match node.kind() {
        ...
        "method_definition" => { pushed = push_named(ctx, node, NodeKind::Method); }
        "class_declaration" | "class" => { pushed = push_named(ctx, node, NodeKind::Class); }
        "variable_declarator" => { pushed = push_named(ctx, node, NodeKind::Variable); }
        "import_statement" => { emit_import(node, ctx); }
...
fn push_named(ctx: &mut Ctx, node: &Node, kind: NodeKind) -> Option<usize> {
    let name_node = node.child_by_field_name("name").or_else(|| {
        ...
        for ch in node.children(&mut c) {
            if matches!(ch.kind(), "identifier" | "property_identifier") { found = Some(ch); break; }
        }
        found
    })?;
    ...
    ctx.result.nodes.push(NodeDraft {
        ...
        signature: sig, docstring: None, language: "javascript".into(),
    });
...
fn emit_call(node: &Node, ctx: &mut Ctx) {
    ...
    let name = match callee.kind() {
        ...
        "member_expression" => callee.child_by_field_name("property").and_then(|p| p.utf8_text(ctx.src).ok()).map(|s| s.to_string()),
        _ => None,
    };
