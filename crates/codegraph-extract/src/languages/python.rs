...
pub struct PythonExtractor { lang: tree_sitter::Language }

impl PythonExtractor {
    pub fn new() -> Self { Self { lang: tree_sitter_python::LANGUAGE.into() } }
}
...
struct Ctx<'a> { src: &'a [u8], result: ExtractResult, parent_idx: Option<usize> }

fn walk(node: &Node, ctx: &mut Ctx) {
    let mut pushed: Option<usize> = None;
    match node.kind() {
        "function_definition" => { pushed = push_named(ctx, node, NodeKind::Function); }
        "class_definition" => { pushed = push_named(ctx, node, NodeKind::Class); }
        "import_statement" | "import_from_statement" => { emit_import(node, ctx); }
        "call" => { emit_call(node, ctx); }
        _ => {}
    }
...
fn push_named(ctx: &mut Ctx, node: &Node, kind: NodeKind) -> Option<usize> {
    ...
    let docstring = node.child_by_field_name("body").and_then(|b| extract_docstring(&b, ctx.src));

    ctx.result.nodes.push(NodeDraft {
        kind, name, qualified_name: None, start_line: start, end_line: end,
        signature: sig, docstring, language: "python".into(),
    });
...
fn extract_docstring(body: &Node, src: &[u8]) -> Option<String> {
    ...
    if s.kind() == "string" {
        ...
        Some(text.trim_matches(|c: char| c == '"' || c == '\'').to_string())
    } else { None }
}
...
fn emit_call(node: &Node, ctx: &mut Ctx) {
    ...
    let name = match callee.kind() {
        ...
        "attribute" => callee.child_by_field_name("attribute").and_then(|a| a.utf8_text(ctx.src).ok()).map(|s| s.to_string()),
        _ => None,
    };
    let Some(n) = name else { return };
    let Some(from) = ctx.parent_idx else { return };
    ctx.result.pending_calls.push(PendingCall { from_idx: from, target_name: n, line: node.start_position().row as u32 + 1 });
}

fn emit_import(node: &Node, ctx: &mut Ctx) {
    let module = if node.kind() == "import_from_statement" {
        node.child_by_field_name("module_name").and_then(|n| n.utf8_text(ctx.src).ok()).map(|s| s.to_string())
    } else {
        let mut c = node.walk();
        let mut found = None;
    ...
    ctx.result.imports.push(RawImport { from_idx: from, module: m, line: node.start_position().row as u32 + 1 });
}
