...
impl HermesTarget {
    fn mcp_path(&self, opts: &InstallOpts) -> Option<Utf8PathBuf> {
        if opts.global {
            ...
            opts.project_root.as_ref().map(|r| r.join(".hermes").join("mcp.json"))
        }
    }
    fn instructions_path(&self, opts: &InstallOpts) -> Option<Utf8PathBuf> {
        if opts.global {
            ...
            opts.project_root.as_ref().map(|r| r.join(".hermes").join("AGENTS.md"))
        }
    }
}
...
impl AgentTarget for HermesTarget {
    ...
    fn label(&self) -> &'static str { "Hermes" }

    fn detect(&self, opts: &InstallOpts) -> DetectStatus {
        let Some(p) = self.mcp_path(opts) else { return DetectStatus::NotFound };
        if !p.exists() { return DetectStatus::NotFound; }
        let Ok(v) = jsonutil::read_or_default(&p) else { return DetectStatus::Found };
        if v.pointer("/mcpServers/codegraph").is_some() { DetectStatus::AlreadyConfigured } else { DetectStatus::Found }
    }

    fn install(&self, opts: &InstallOpts) -> Result<InstallReport> {
        let mcp = self.mcp_path(opts).ok_or_else(|| anyhow::anyhow!("no hermes path"))?;
        ...
        {
            let obj = v.as_object_mut().ok_or_else(|| anyhow::anyhow!("hermes config not an object"))?;
        ...
        if changed { jsonutil::write_pretty(&mcp, &v)?; written.push(mcp); }
    ...
    fn uninstall(&self, opts: &InstallOpts) -> Result<InstallReport> {
        let Some(mcp) = self.mcp_path(opts) else { return Ok(InstallReport::Unchanged) };
        if !mcp.exists() { return Ok(InstallReport::Unchanged); }
        let mut v = jsonutil::read_or_default(&mcp)?;
        let mut changed = false;
