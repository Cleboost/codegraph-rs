...
impl OpencodeTarget {
    ...
    fn parse_jsonc(&self, text: &str) -> Result<Value> {
        if text.trim().is_empty() { return Ok(Value::Object(Default::default())); }
...
impl AgentTarget for OpencodeTarget {
    ...
    fn label(&self) -> &'static str { "opencode" }

    fn detect(&self, opts: &InstallOpts) -> DetectStatus {
        let Some(p) = self.config_path(opts) else { return DetectStatus::NotFound };
        if !p.exists() { return DetectStatus::NotFound; }
        let Ok(text) = std::fs::read_to_string(p.as_std_path()) else { return DetectStatus::Found };
        let Ok(v) = self.parse_jsonc(&text) else { return DetectStatus::Found };
        if v.pointer("/mcp/codegraph").is_some() { DetectStatus::AlreadyConfigured } else { DetectStatus::Found }
    }

    fn install(&self, opts: &InstallOpts) -> Result<InstallReport> {
        let config = self.config_path(opts).ok_or_else(|| anyhow::anyhow!("no opencode config path"))?;
        let text = std::fs::read_to_string(config.as_std_path()).unwrap_or_default();
        let mut v = self.parse_jsonc(&text)?;

        let mut cmd: Vec<Value> = vec![Value::String(opts.binary_path.to_string()), Value::String("serve".into()), Value::String("--mcp".into())];
        ...
        {
            let obj = v.as_object_mut().ok_or_else(|| anyhow::anyhow!("opencode config not an object"))?;
            let mcp = obj.entry("mcp").or_insert_with(|| Value::Object(Default::default()));
            let mcp = mcp.as_object_mut().ok_or_else(|| anyhow::anyhow!("mcp not an object"))?;
    ...
    fn uninstall(&self, opts: &InstallOpts) -> Result<InstallReport> {
        let Some(config) = self.config_path(opts) else { return Ok(InstallReport::Unchanged) };
        ...
        if let Some(mcp) = v.pointer_mut("/mcp").and_then(|m| m.as_object_mut()) {
            if mcp.remove("codegraph").is_some() { changed = true; }
        }
        if changed {
            jsonutil::write_pretty(&config, &v)?;
