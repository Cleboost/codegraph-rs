...
pub fn tool_definitions() -> Vec<Value> {
    vec![
        tool("codegraph_search", "Search the knowledge graph by name / signature / docstring (FTS5).",
            json!({ "type": "object", "properties": {
                "query": { "type": "string" },
                "limit": { "type": "integer", "default": 20 }
            }, "required": ["query"] })),
        tool("codegraph_node", "Look up a node by id or exact name.",
            json!({ "type": "object", "properties": {
                "id": { "type": "integer" },
                "name": { "type": "string" }
            } })),
        ...
        tool("codegraph_callees", "Find functions called by the given node.",
            json!({ "type": "object", "properties": {
                "node": { "type": "integer" },
                "depth": { "type": "integer", "default": 1 }
            }, "required": ["node"] })),
        tool("codegraph_impact", "Impact radius: who transitively depends on this node.",
            json!({ "type": "object", "properties": {
                "node": { "type": "integer" },
                "max_depth": { "type": "integer", "default": 3 }
            }, "required": ["node"] })),
        tool("codegraph_context", "Composed context for a symbol or topic (search + callers + callees).",
            json!({ "type": "object", "properties": {
                "query": { "type": "string" },
                "depth": { "type": "integer", "default": 1 },
        ...
        tool("codegraph_files", "List indexed files under a path prefix.",
            json!({ "type": "object", "properties": { "path": { "type": "string" } } })),
        tool("codegraph_status", "Index health: counts, size, schema version.",
            json!({ "type": "object", "properties": {} })),
    ]
}
    ...
    match name {
        ...
        "codegraph_context" => {
            let req = ContextRequest {
                ...
                include_source: args.get("include_source").and_then(|v| v.as_bool()).unwrap_or(false),
        ...
        "codegraph_status" => {
            Ok(serde_json::to_string_pretty(&db.stats()?)?)
        }
...
fn arg_str<'a>(v: &'a Value, k: &str) -> anyhow::Result<&'a str> {
    v.get(k).and_then(|x| x.as_str()).ok_or_else(|| anyhow::anyhow!("missing string arg: {k}"))
}
fn arg_i64(v: &Value, k: &str) -> anyhow::Result<i64> {
    v.get(k).and_then(|x| x.as_i64()).ok_or_else(|| anyhow::anyhow!("missing int arg: {k}"))
}
