...
impl<'a> Resolver<'a> {
    ...
    pub fn resolve_calls(&self, pending: &[PendingCallRow]) -> Result<usize> {
        ...
        for p in pending { by_name.entry(p.target_name.as_str()).or_default().push(p); }
        ...
        for (name, sites) in by_name {
            ...
            for site in sites {
                ...
                let same_file: Vec<_> = callable
                    .iter()
                    .filter(|n| {
                        self.db.file_by_path(n.file.as_str())
                            .ok().flatten()
                            .and_then(|f| f.id)
